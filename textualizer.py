    ...
    if question_mapping is not None and responses_dict is not None and open_var_options and variable:
        ...
        with tabs[3]:
            ...
            if not group_by:
                ...
                with viz_tabs[0]:
                    st.plotly_chart(
                        create_sentiment_sunburst(sentiment_stats),
                        use_container_width=True
                    )
        ...
        # Organize responses with or without grouping
        if group_by:
            # Get texts by group
            texts_by_group = {}
            for df in responses_dict.values():
            ...
            for df in responses_dict.values():
                if variable in df.columns:
                    texts = df[variable].dropna().tolist()
                    texts_by_group['All Responses'].extend(texts)
